...
class Bootstrap {
   ...
   */
  public string $version = '7.78';

  /**
   * Core API compatibility.
   */
  public string $drupal_core_compatibility = '7.x';

  /**
   * Minimum supported version of PHP.
   */
  public string $drupal_minimum_php = '5.2.4';

  /**
   * Minimum recommended value of PHP memory_limit.
   */
  public string $drupal_minimum_php_memory_limit = '32M';

  /**
   * Error reporting level: display no errors.
   */
  public int $error_reporting_hide = 0;

  /**
   * Error reporting level: display errors and warnings.
   */
  public int $error_reporting_display_some = 1;

  /**
   * Error reporting level: display all messages.
   */
  public int $error_reporting_display_all = 2;
   ...
   */
  public int $cache_permanent = 0;

  /**
   * Indicates that the item should be removed at the next general cache wipe.
   */
  public int $cache_temporary = -1;
   ...
   */
  public int $watchdog_emergency = 0;

  /**
   * Log message severity -- Alert: action must be taken immediately.
   */
  public int $watchdog_alert = 1;

  /**
   * Log message severity -- Critical conditions.
   */
  public int $watchdog_critical = 2;

  /**
   * Log message severity -- Error conditions.
   */
  public int $watchdog_error = 3;

  /**
   * Log message severity -- Warning conditions.
   */
  public int $watchdog_warning = 4;

  /**
   * Log message severity -- Normal but significant conditions.
   */
  public int $watchdog_notice = 5;

  /**
   * Log message severity -- Informational messages.
   */
  public int $watchdog_info = 6;

  /**
   * Log message severity -- Debug-level messages.
   */
  public int $watchdog_debug = 7;
   ...
   */
  public int $drupal_bootstrap_configuration = 0;

  /**
   * Second bootstrap phase: try to serve a cached page.
   */
  public int $drupal_bootstrap_page_cache = 1;

  /**
   * Third bootstrap phase: initialize database layer.
   */
  public int $drupal_bootstrap_database = 2;

  /**
   * Fourth bootstrap phase: initialize the variable system.
   */
  public int $drupal_bootstrap_variables = 3;

  /**
   * Fifth bootstrap phase: initialize session handling.
   */
  public int $drupal_bootstrap_session = 4;

  /**
   * Sixth bootstrap phase: set up the page header.
   */
  public int $drupal_bootstrap_page_header = 5;

  /**
   * Seventh bootstrap phase: find out language of the page.
   */
  public int $drupal_bootstrap_language = 6;

  /**
   * Final bootstrap phase: Drupal is fully loaded; validate and fix input data.
   */
  public int $drupal_bootstrap_full = 7;

  /**
   * Role ID for anonymous users; should match what's in the "role" table.
   */
  public int $drupal_anonymous_rid = 1;

  /**
   * Role ID for authenticated users; should match what's in the "role" table.
   */
  public int $drupal_authenticated_rid = 2;
   ...
   */
  public int $drupal_kilobyte = 1024;
   ...
   */
  public string $language_none = 'und';

  /**
   * The type of language used to define the content language.
   */
  public string $language_type_content = 'language_content';

  /**
   * The type of language used to select the user interface.
   */
  public string $language_type_interface = 'language';

  /**
   * The type of language used for URLs.
   */
  public string $language_type_url = 'language_url';

  /**
   * Language written left to right. Possible value of public int $language->direction.
   */
  public int $language_ltr = 0;

  /**
   * Language written right to left. Possible value of public int $language->direction.
   */
  public int $language_rtl = 1;

  /**
   * Time of the current request in seconds elapsed since the Unix Epoch.
   *
   * This differs from public int $_SERVER['REQUEST_TIME'], which is stored as a float
   ...
   */
  public int $registry_reset_lookup_cache = 1;

  /**
   * Signals that the registry lookup cache should be written to storage.
   */
  public int $registry_write_lookup_cache = 2;
   ...
   */
  public string $drupal_php_function_pattern = '[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*';
  ...
  public string $date_rfc7231 = 'D, d M Y H:i:s \G\M\T';

  public string $base_url, $base_path, $base_root;

  public string $databases, $cookie_domain, $conf, $installed_profile, $update_free_access, $db_url, $db_prefix, $drupal_hash_salt, $is_https, $base_secure_url, $base_insecure_url;
  ...
  function drupal_environment_initialize() {
    ...
    if (isset($_SERVER['HTTP_HOST'])) {
      ...
      $_SERVER['HTTP_HOST'] = strtolower($_SERVER['HTTP_HOST']);
       if (!$this->drupal_valid_http_host($_SERVER['HTTP_HOST'])) {
  ...
  function drupal_settings_initialize() {
    if (file_exists(DRUPAL_ROOT . '/' . $this->conf_path() . '/settings.php')) {
      include_once DRUPAL_ROOT . '/' . $this->conf_path() . '/settings.php';
    }
    $is_https = $this->drupal_is_https();
  ...
  function drupal_bootstrap($phase = NULL, $new_phase = TRUE) {
    ...
    $phases = array(
      $this->drupal_bootstrap_configuration,
      $this->drupal_bootstrap_page_cache,
      $this->drupal_bootstrap_database,
      $this->drupal_bootstrap_variables,
      $this->drupal_bootstrap_session,
      $this->drupal_bootstrap_page_header,
      $this->drupal_bootstrap_language,
      $this->drupal_bootstrap_full,
    );
    ...
    if (isset($phase)) {
      ...
      while ($phases && $phase > $stored_phase && $final_phase > $stored_phase) {
        ...
        switch ($current_phase) {
          case $this->drupal_bootstrap_configuration:
  ...
  function drupal_fast_404() {
    ...
    if ($exclude_paths && !preg_match($exclude_paths, $_GET['q'])) {
      ...
      if ($fast_paths && preg_match($fast_paths, $_GET['q'])) {
        $this->drupal_add_http_header('Status', '404 Not Found');
        $fast_404_html = $this->variable_get(
          '404_fast_html',
